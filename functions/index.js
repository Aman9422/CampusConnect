...
// ===============================================
// v8.8.2 (A, HIGH): CRASH-SAFE QUOTA COMPENSATION
// ===============================================
...
/**
 * v8.8.2 (A, HIGH): refund credits whose reservation was left stale by a
 * crash/500 in `reviewResume`.
 *
...
// ===============================================
// V8.8 (P5/P6): AI CHAT DELETION + RETENTION
// ===============================================
 ...
 * per batch) and idempotent. `admin.database`/`admin.firestore` Admin SDK
 * bypasses security rules, so the owner-scope enforcement is explicit here
 * (queries are scoped to the auth uid).
 *
 * @returns {Promise<{deleted: number}>}
 */
exports.deleteAIHistory = onCall(
    // v8.8.3 (MED-8): a 120 s timeout matches the client's delete call — a
    // large history can take several Firestore batch round-trips, and the
    // default 60 s callable timeout could abort the deletion mid-way.
    {maxInstances: 10, timeoutSeconds: 120},
    async (request) => {
      const userId = request.auth?.uid;
      if (!userId) {
      ...
      try {
        ...
        // Aggregate only — never log conversation contents.
        return {deleted};
      } catch (error) {
 ...
 *
 * Conservative by design: RECENT conversations are never touched, and no
 * other Firestore documents are ever considered. Deletes are batched
 * (≤400/batch), idempotent, and safe for large datasets. Only aggregate
 * counts are logged — never prompts, responses, or personal content.
 *
...
exports.cleanupExpiredAIConversations = onSchedule(
    ...
    async () => {
      ...
      try {
        // 1) users/{uid}/ai_interactions — collection group query across all
        // users (single-field orderBy on createdAt; automatic single-field
        // index is provisioned, no composite needed).
        const interactionsSnapshot = await admin.firestore()
        ...
        // 2) legacy ai_conversations store.
        const conversationsSnapshot = await admin.firestore()
            .collection("ai_conversations")
