...
class _EditProfileViewState extends State<EditProfileView> {
  ...
  static const _employmentTypes = ['Full-time', 'Part-time', 'Contract', 'Freelance', 'Self-employed'];
  ...
  void initState() {
    ...
    _fullNameController = TextEditingController(text: profile?.personal.fullName ?? '');
    ...
    _collegeController = TextEditingController(text: profile?.academic.college ?? '');
    _programController = TextEditingController(text: profile?.academic.program ?? '');
    _cgpaController = TextEditingController(
      text: (profile?.academic.cgpa == 0.0 || profile?.academic.cgpa == null) ? '' : profile!.academic.cgpa.toString(),
    );
    ...
    _linkedinController = TextEditingController(text: profile?.linkedinProfile ?? '');
    _industryController = TextEditingController(text: profile?.industry ?? '');
    _workLocationController = TextEditingController(text: profile?.workLocation ?? '');
    ...
    _leetcodeController = TextEditingController(text: profile?.leetcodeUrl ?? '');
    _hackerrankController = TextEditingController(text: profile?.hackerrankUrl ?? '');
  ...
  InputDecoration _inputDecoration(bool isDark, String? hintText) {
    return InputDecoration(
      ...
      border: OutlineInputBorder(
        ...
        borderSide: BorderSide(color: isDark ? AppTheme.gray700 : AppTheme.gray300),
      ),
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Text(
            label,
            style: AppTheme.label.copyWith(fontWeight: FontWeight.w600, color: isDark ? Colors.white : AppTheme.gray800),
          ),
  ...
  Future<void> _saveProfile() async {
    ...
    final isAlumni = context.read<RoleProvider>().userRole == UserRole.alumni;

    if (currentProfile == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Profile not loaded')));
      return;
    }
    ...
    final updatedProfile = isAlumni
        ? currentProfile.copyWith(
            ...
            jobRole: _jobRoleController.text.trim().isEmpty ? null : _jobRoleController.text.trim(),
            ...
            leetcodeUrl: _leetcodeController.text.trim().isEmpty ? null : _leetcodeController.text.trim(),
            hackerrankUrl: _hackerrankController.text.trim().isEmpty ? null : _hackerrankController.text.trim(),
            skills: skills,
          )
        : currentProfile.copyWith(
            personal: currentProfile.personal.copyWith(
    ...
    if (mounted) {
      if (success) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(profileProvider.error ?? 'Failed to update profile'), backgroundColor: AppTheme.error),
        );
      }
    }
  }
            ...
            child: Form(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _field('Full Name', _fullNameController, TextInputType.text, validator: (v) {
                    if (v == null || v.trim().isEmpty) return 'Please enter your full name';
                    return null;
                  }),
                  _field('Phone Number', _phoneController, TextInputType.phone, hint: 'Optional'),
                  _field('Bio', _bioController, TextInputType.text, hint: 'Short bio (optional)'),
                  ...
                  if (isAlumni) ...[
                    ...
                    _field('Company', _companyController, TextInputType.text, hint: 'Current company'),
                    _field('Job Title', _jobRoleController, TextInputType.text, hint: 'Current job title'),

                    // Employment Type dropdown
                    Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text('Employment Type', style: AppTheme.label.copyWith(fontWeight: FontWeight.w600, color: isDark ? Colors.white : AppTheme.gray800)),
                        const SizedBox(height: AppTheme.space8),
                        DropdownButtonFormField<String>(
                          ...
                          items: _employmentTypes.map((t) => DropdownMenuItem(value: t, child: Text(t, style: TextStyle(color: isDark ? Colors.white : AppTheme.gray900)))).toList(),
                    ...
                    Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text('Work Mode', style: AppTheme.label.copyWith(fontWeight: FontWeight.w600, color: isDark ? Colors.white : AppTheme.gray800)),
                        const SizedBox(height: AppTheme.space8),
                        DropdownButtonFormField<String>(
                          value: _workMode,
                          dropdownColor: isDark ? AppTheme.darkSurface : Colors.white,
                          style: TextStyle(color: isDark ? Colors.white : AppTheme.gray900),
                          decoration: _inputDecoration(isDark, null),
                          items: _workModes.map((m) => DropdownMenuItem(value: m, child: Text(m, style: TextStyle(color: isDark ? Colors.white : AppTheme.gray900)))).toList(),
                    ...
                    _field('Industry', _industryController, TextInputType.text, hint: 'e.g., Software, Finance'),
                    _field('Work Location', _workLocationController, TextInputType.text, hint: 'City / Region'),
                    const SizedBox(height: AppTheme.space8),

                    // ── Skills (alumni) ──
                    _sectionHeader('Skills & Expertise', isDark),
                    _field('Skills', _skillsController, TextInputType.text, hint: 'Comma-separated, e.g., Flutter, Python, React'),
                    const SizedBox(height: AppTheme.space8),

                    // ── Social Links (alumni) ──
                    _sectionHeader('Social Links', isDark),
                    _field('LinkedIn URL', _linkedinController, TextInputType.url, hint: 'https://linkedin.com/in/...'),
                    _field('GitHub URL', _githubController, TextInputType.url, hint: 'https://github.com/...'),
                    _field('Portfolio URL', _portfolioController, TextInputType.url, hint: 'https://...'),
                    _field('Website URL', _websiteController, TextInputType.url, hint: 'https://...'),
                    _field('LeetCode URL', _leetcodeController, TextInputType.url, hint: 'https://leetcode.com/...'),
                    _field('HackerRank URL', _hackerrankController, TextInputType.url, hint: 'https://hackerrank.com/...'),
                  ] else ...[
                    const SizedBox(height: AppTheme.space8),

                    // ── Academic Information (student/teacher only) ──
                    _sectionHeader('Academic Information', isDark),
                    _field('College', _collegeController, TextInputType.text, validator: (v) {
                      if (v == null || v.trim().isEmpty) return 'Please enter your college name';
                      return null;
                    }),
                    _field('Program', _programController, TextInputType.text, hint: 'e.g., Computer Science', validator: (v) {
                      if (v == null || v.trim().isEmpty) return 'Please enter your program';
                      return null;
                    }),
                    ...
                    Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text('Year', style: AppTheme.label.copyWith(fontWeight: FontWeight.w600, color: isDark ? Colors.white : AppTheme.gray800)),
                        const SizedBox(height: AppTheme.space8),
                        DropdownButtonFormField<int>(
                          ...
                          items: [1, 2, 3, 4].map((year) => DropdownMenuItem(
                            value: year,
                            child: Text('Year $year', style: TextStyle(color: isDark ? Colors.white : AppTheme.gray900)),
                          )).toList(),
                    ...
                    _field('CGPA', _cgpaController, const TextInputType.numberWithOptions(decimal: true), hint: 'e.g., 8.5', validator: (v) {
                      if (v == null || v.trim().isEmpty) return 'Please enter your CGPA';
