...
class _AIInsightsTabState extends State<AIInsightsTab> {
    ...
    return _card(
      ...
      child: Wrap(
        ...
        children: [
          ...
          _healthTile('Engagement', '${analytics.avgEngagement}', AppTheme.success, isDark),
          _healthTile('Profile Strength', '${analytics.avgProfileStrength}', AppTheme.primaryBlue, isDark),
          _healthTile('Alumni', '${analytics.activeAlumni}', const Color(0xFF7C3AED), isDark),
        ],
      ),
    );
  }
  ...
  Widget _healthTile(String label, String value, Color color, bool isDark) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        final tileWidth = (constraints.maxWidth - 24) / 4; // 4 cols with 3 gaps of 8
        return Container(
  ...
  // ─── 2. Resume Distribution (PieChart) ────────────
  Widget _buildResumeDistributionChart(TeacherAnalyticsProvider analytics, bool isDark) {
    final total = analytics.totalReviews;
    if (total == 0) {
      return _card('Resume Distribution', Icons.pie_chart_outline, isDark,
          child: Text('No review data yet',
    ...
    return _card('Resume Distribution', Icons.pie_chart_outline, isDark,
      ...
      children: [
        ...
        Wrap(
          ...
          runSpacing: 6,
          children: [
            _legendDot('Excellent (80+)', const Color(0xFF059669), excellent.round()),
  ...
  Widget _buildPlacementFunnelChart(TeacherAnalyticsProvider analytics, bool isDark) {
    ...
    final eligible = pipelineData?.eligibleStudents ?? analytics.studentData?.length ?? 0;
    final applied = pipelineData?.appliedStudents ?? 0;

    if (eligible == 0) {
      return _card('Placement Funnel', Icons.share_arrival_time_outlined, isDark,
          child: Text('No placement data available',
              style: AppTheme.bodySmall.copyWith(color: isDark ? AppTheme.gray400 : AppTheme.gray500)));
    }

    // Dynamic Y-axis: 20% padding above the max value — never hardcoded
    final maxValue = [eligible, applied, 0, 0, 0]
        .reduce((a, b) => a > b ? a : b)
        .toDouble();
        ...
        SizedBox(
          ...
          child: BarChart(
            BarChartData(
              ...
              barGroups: [
                _barGroup(0, eligible.toDouble(), AppTheme.primaryBlue, 'Eligible'),
                _barGroup(1, applied.toDouble(), AppTheme.secondaryIndigo, 'Applied'),
                ...
                bottomTitles: AxisTitles(
                  sideTitles: SideTitles(
                    ...
                    getTitlesWidget: (value, meta) {
                      const labels = ['Eligible', 'Applied', 'Short.', 'Interview', 'Placed'];
                      final idx = value.toInt();
                      if (idx < 0 || idx >= labels.length) return const SizedBox.shrink();
                      return Padding(
                        padding: const EdgeInsets.only(top: 6),
                        child: Text(labels[idx], style: TextStyle(fontSize: 9, color: isDark ? Colors.white70 : Colors.black54)),
                      );
                    },
                  ),
                ),
              ...
              barTouchData: BarTouchData(
                touchTooltipData: BarTouchTooltipData(
                  getTooltipItem: (group, groupIndex, rod, rodIndex) {
                    ...
                    final label = idx >= 0 && idx < longLabels.length ? longLabels[idx] : '';
  ...
  // ─── 4. Department Comparison (BarChart) ──────────
  Widget _buildDepartmentComparisonChart(TeacherAnalyticsProvider analytics, bool isDark) {
    final depts = analytics.departmentAnalytics ?? [];
    if (depts.isEmpty) {
      return _card('Department Comparison', Icons.account_balance_outlined, isDark,
              ...
              titlesData: FlTitlesData(
                ...
                bottomTitles: AxisTitles(
                  sideTitles: SideTitles(
                    ...
                    getTitlesWidget: (value, meta) {
                      final idx = value.toInt();
                      if (idx < 0 || idx >= displayDepts.length) return const SizedBox.shrink();
                      final name = (displayDepts[idx]['department'] as String? ?? '');
                      final short = name.length > 8 ? '${name.substring(0, 7)}.' : name;
                      return Padding(
                        padding: const EdgeInsets.only(top: 6),
                        child: Text(short, style: TextStyle(fontSize: 9, color: isDark ? Colors.white70 : Colors.black54)),
                      );
                    },
                  ),
                ),
                ...
                touchTooltipData: BarTouchTooltipData(
                  getTooltipItem: (group, groupIndex, rod, rodIndex) {
                    ...
                    final deptName = idx >= 0 && idx < displayDepts.length
                        ...
                        : '';
                    return BarTooltipItem('$deptName\n${rod.toY.round()}/100',
                        const TextStyle(color: Colors.white, fontSize: 10));
                  },
                ),
              ),
  ...
  Widget _buildSkillGapChart(TeacherAnalyticsProvider analytics, bool isDark) {
    ...
    if (gaps.isEmpty) {
      return _card('Skill Gap Distribution', Icons.psychology_outlined, isDark,
    ...
    final maxCount = displayGaps.fold<int>(
        0, (m, g) => (g['count'] as int? ?? 0) > m ? (g['count'] as int? ?? 0) : m);
              ...
              titlesData: FlTitlesData(
                ...
                bottomTitles: AxisTitles(
                  sideTitles: SideTitles(
                    ...
                    getTitlesWidget: (value, meta) {
                      final idx = value.toInt();
                      if (idx < 0 || idx >= displayGaps.length) return const SizedBox.shrink();
                      final skill = (displayGaps[idx]['skill'] as String? ?? '');
                      final short = skill.length > 7 ? '${skill.substring(0, 6)}.' : skill;
                      return Padding(
                        padding: const EdgeInsets.only(top: 6),
                        child: Text(short, style: TextStyle(fontSize: 9, color: isDark ? Colors.white70 : Colors.black54)),
                      );
  ...
  // ─── 8. Risk Distribution (PieChart) ──────────────
  Widget _buildRiskDistributionChart(TeacherAnalyticsProvider analytics, bool isDark) {
    ...
    if (total == 0) {
      return _card('Risk Distribution', Icons.warning_amber_rounded, isDark,
          child: Text('No risk data',
              style: AppTheme.bodySmall.copyWith(color: isDark ? AppTheme.gray400 : AppTheme.gray500)));
    }
    ...
    return _card('Risk Distribution', Icons.warning_amber_rounded, isDark,
      ...
      children: [
        ...
        Wrap(
          spacing: 12,
          children: [
            _legendDot('High Potential', AppTheme.success, highPotential.round()),
            _legendDot('Medium', AppTheme.warning, medium.round()),
            _legendDot('At Risk', AppTheme.error, atRisk.round()),
          ],
        ),
        const SizedBox(height: AppTheme.space8),
        Text('Student Health Score: $healthScore%',
            style: AppTheme.bodySmall.copyWith(
                fontWeight: FontWeight.w600,
  ...
  // ─── 9. Monthly Resume Trend (LineChart) ──────────
  Widget _buildMonthlyTrendChart(TeacherAnalyticsProvider analytics, bool isDark) {
    final trends = analytics.performanceTrends ?? [];
    if (trends.isEmpty) {
      return _card('Monthly Resume Trend', Icons.show_chart_rounded, isDark,
                ...
                bottomTitles: AxisTitles(
                  sideTitles: SideTitles(
                    ...
                    getTitlesWidget: (value, meta) {
                      ...
                      final short = month.length >= 7 ? month.substring(5) : month;
                      return Padding(
  ...
  Widget _buildAISummary(TeacherAnalyticsProvider analytics, BuildContext context, bool isDark) {
    ...
    if (depts.length >= 2) {
      final sortedAsc = List<Map<String, dynamic>>.from(depts)
        ..sort((a, b) => ((a['avgScore'] as int? ?? 0)).compareTo(b['avgScore'] as int? ?? 0));
    ...
    // Department insights
    if (topDept.isNotEmpty && weakestDept.isNotEmpty && topDept != weakestDept) {
      paragraphs.add(
    ...
    if (avgEngagement > 0 || avgProfileStrength > 0) {
      final engagementDir = avgEngagement >= 60 ? 'positive' : 'needs improvement';
      paragraphs.add(
    ...
    if (paragraphs.isEmpty) {
      paragraphs.add('No analytics data available yet. Start by reviewing student resumes to generate insights.');
    }
