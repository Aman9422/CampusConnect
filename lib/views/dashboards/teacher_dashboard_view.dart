...
class _TeacherDashboardBodyState extends State<_TeacherDashboardBody> {
  ...
  Widget _buildHeroSection(
    ...
    final activePlacements = placementsProvider.placements
        ...
        .length;
    final placementRate =
        totalPlacements > 0 ? (activePlacements / totalPlacements * 100) : 0;
        ...
        children: [
          ...
          Row(
            children: [
              ...
              _heroStat(
                totalPlacements.toString(),
                'Total Drives',
                Icons.business_outlined,
              ),
              const SizedBox(width: AppTheme.space12),
              _heroStat(
                '${placementRate.toStringAsFixed(0)}%',
                'Placement Rate',
  ...
  Widget _buildAnalyticsCards(
      BuildContext context, TeacherAnalyticsProvider analytics, bool isDark) {
          ...
          children: [
            Expanded(
              child: _buildAnalyticCard(
                'Students',
                analytics.studentData?.length.toString() ?? '0',
        ...
        Row(
          children: [
            ...
            Expanded(
              child: _buildAnalyticCard(
                'Placement Ready',
                '${analytics.predictedPlacementRate.round()}%',
      ...
      children: [
        ...
        if (analytics.hasData && analytics.totalReviews > 0)
          _buildChartCard(
            ...
            SizedBox(
              height: 200,
              child: _ResumeScorePieChart(analytics: analytics, isDark: isDark),
            ),
            isDark,
          ),
        if (analytics.hasData && analytics.totalReviews > 0)
        ...
        // Skill Gap Summary
        if (analytics.skillGapAnalysis != null &&
            analytics.skillGapAnalysis!.isNotEmpty)
          _buildChartCard(
            'Skill Gap Summary',
            _buildSkillGapBars(analytics, isDark),
            isDark,
          ),
        if (analytics.skillGapAnalysis != null &&
            analytics.skillGapAnalysis!.isNotEmpty)
        ...
        _buildChartCard(
          'Placement Pipeline',
          _buildPlacementPipelineChart(context, isDark),
          isDark,
        ),
      ],
    );
  ...
  Widget _buildPlacementPipelineChart(BuildContext context, bool isDark) {
    ...
    final upcoming = placements.where((p) =>
        p.isActive && !p.isDeadlinePassed && p.deadline.difference(now).inDays > 7).length;
        ...
        child: Center(
          child: Text('No skill gap data',
              style: AppTheme.caption
                  .copyWith(color: isDark ? AppTheme.gray400 : AppTheme.gray500)),
        ),
      );
    }
  ...
  Widget _buildAlertsSection(
    ...
    if (analytics.atRiskCount > 0) {
      alerts.add(_AlertItem(
        ...
        'Students needing placement support',
    ...
    if (analytics.poorCount > 0) {
      alerts.add(_AlertItem(
        '${analytics.poorCount} low resume scores',
        'Scores below 40, need improvement',
    ...
    if (analytics.highPotentialCount > 0) {
      alerts.add(_AlertItem(
        '${analytics.highPotentialCount} high potential',
  ...
  Widget _buildAcademicTools(BuildContext context, bool isDark) {
    final tools = [
      _ToolItem('Analytics', Icons.analytics_outlined, AppTheme.primaryBlue,
          studentAnalyticsRoute),
      _ToolItem('Students', Icons.people_outline, AppTheme.success,
          studentAnalyticsRoute),
      _ToolItem(
          'Resume Reviews',
          Icons.description_outlined,
          AppTheme.warning,
          resumeReviewRoute),
      _ToolItem('AI Insights', Icons.auto_awesome, AppTheme.secondaryIndigo,
          aiChatRoute),
      _ToolItem('Reports', Icons.assessment_outlined, AppTheme.primaryBlue,
          studentAnalyticsRoute),
      _ToolItem(
          'Notifications',
  ...
  Widget _buildRecentActivities(BuildContext context, bool isDark) {
    ...
    final activities = <_ActivityItem>[
      if (reviewsProvider.history.isNotEmpty)
        _ActivityItem(
          '${reviewsProvider.history.length} resume reviews completed',
          'Last: ${reviewsProvider.history.first.atsScore}/100',
          Icons.description_outlined,
          AppTheme.success,
        ),
      ...activityFeed.allActivities.take(5).map((a) => _ActivityItem(
            a.title,
            a.description,
            a.icon,
            a.iconColor,
...
class _ResumeScorePieChart extends StatelessWidget {
  ...
  const _ResumeScorePieChart(
      {required this.analytics, required this.isDark});
      ...
      children: [
            ...
            PieChartData(
              ...
              sections: [
                PieChartSectionData(
                  ...
                  value: excellentPct,
                  title: '${analytics.excellentCount}',
                  radius: 55,
        ...
        Expanded(
          ...
          child: Column(
            ...
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              _legendItem('Excellent (80+)', analytics.excellentCount,
                  const Color(0xFF059669), isDark),
              const SizedBox(height: AppTheme.space8),
              _legendItem('Good (60-79)', analytics.goodCount,
                  const Color(0xFF0891B2), isDark),
              const SizedBox(height: AppTheme.space8),
              _legendItem('Fair (40-59)', analytics.fairCount,
                  const Color(0xFFEA580C), isDark),
              const SizedBox(height: AppTheme.space8),
              _legendItem('Poor (<40)', analytics.poorCount,
